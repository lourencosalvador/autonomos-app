...
export default function PerfilPrestadorScreen() {
  ...
  const openChat = () =>
    router.push({ pathname: '/chat', params: { otherUserId: providerId, otherUserName: displayName } });
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }} bounces={false}>
        ...
        <View style={{ height: HERO_H, width: '100%', backgroundColor: '#E5E7EB' }}>
          ...
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, paddingTop: 56, paddingHorizontal: 18 }} className="flex-row items-center justify-between">
            ...
            <TouchableOpacity onPress={openChat} activeOpacity={0.8} className="h-11 w-11 items-center justify-center rounded-full" style={{ backgroundColor: 'rgba(0,0,0,0.30)' }}>
        ...
        <View className="rounded-t-[28px] bg-white px-5 pt-5" style={{ marginTop: -26 }}>
          ...
          <View className="mt-1 flex-row items-start justify-between">
            ...
            <View className="items-end">
              ...
              <Text className="text-[18px] font-extrabold text-brand-cyan">A combinar</Text>
            </View>
          </View>
          ...
          <View className="mt-3 flex-row items-center justify-between">
            ...
            <TouchableOpacity onPress={openChat} activeOpacity={0.85} className="h-10 w-10 items-center justify-center rounded-full bg-gray-100">
      ...
      >
        <TouchableOpacity onPress={openChat} activeOpacity={0.85} className="flex-1 flex-row items-center justify-center gap-2 rounded-full" style={{ height: 54, borderWidth: 1.5, borderColor: '#00E7FF' }}>
          <Ionicons name="chatbubble-ellipses-outline" size={18} color="#00A9BA" />
          <Text className="text-[14px] font-extrabold text-cyan-700">Mensagem</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={startRequest} activeOpacity={0.85} className="flex-1 flex-row items-center justify-center gap-2 rounded-full bg-brand-cyan" style={{ height: 54 }}>
