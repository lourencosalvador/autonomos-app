...
function base64ToUint8Array(base64: string) {
  // RN/Expo tem atob disponível; fallback simples caso não exista.
...
export async function uploadAvatarToSupabase(params: { userId: string; uri: string }) {
  ...
  const base64Encoding =
    ((FileSystem as any).EncodingType?.Base64 as string | undefined) || ('base64' as const);
  ...
  if (!publicUrl) throw new Error('Não foi possível obter URL pública do avatar.');
